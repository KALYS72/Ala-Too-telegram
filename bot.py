...
<b>Change the group with elective</b> - Explains itself 
"""
    week = record_get("source.json")
...
def process_review(message):
    ...
    reviews = record_get('reviews.json')
    if review_text != '/review' or review_text != '/info' or review_text != '/schedule' or review_text != '/text':
...
def get_electives(message, user, group, back=None, electives=[], again=False):
    ...
    submit_button = types.InlineKeyboardButton(text="Submit", callback_data=f"confirm_{back[1]}")
...
def schedule_menu(message, user=None, back=None):
    ...
    button_change_group = types.InlineKeyboardButton(text="Change group", callback_data='change_group')
...
def confirm(call):
    ...
    try:
        bot.delete_message(chat_id=call.message.chat.id, message_id=mess)
    except:
        print('confirm')
    schedule_menu(call.message, user=str(call.from_user.id), back=(call.message.chat.id, call.message.message_id))
